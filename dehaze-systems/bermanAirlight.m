function [ Aout ] = bermanAirlight( img, Amin, Amax, N, spacing, K, thres)
...
if ~exist('n_colors','var') || isempty(N), N = 500 ; end; %number of colors clusters
if ~exist('K','var') || isempty(K), K = 20 ; end; %number of angles
...
p = int16(floor(points.*500));
S = sparse(p(:,1),p(:,2),points_weight);
assignin('base','S',S);
F = full(S);
[~, AvoteRG] = vote_2D(points(:,1:2), points_weight, directions_all, Aall, thres );
assignin('base','vote',AvoteRG);
% Estimate GB
